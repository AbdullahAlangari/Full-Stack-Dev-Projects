...
def venues():
  ...
  venues = db.session.query(Venue).order_by('state').order_by('city').all()
  cities = []
...
def search_venues():
  ...
  venues =db.session.query(Venue).filter(func.lower(Venue.name).contains(func.lower(request.form.get('search_term')))).all()
  response={
    "count": len(venues),
    "data": venues
  }
...
def artists():
  ...
  #Query venues, case insensitive, including ones that contains the search term 
  venues = db.session.query(Venue).filter(func.lower(Venue.name).contains(func.lower(request.form.get('search_term')))).all()
...
def search_artists():
  ...
  # Query artists, case insensitive, including ones that contains the search term 
  artists = db.session.query(Artist).filter(func.lower(Artist.name).contains(func.lower(request.form.get('search_term')))).all()
  response={
    "count": len(artists),
    "data": artists
  }
...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  db_shows = db.session.query(Show).all()
  if len(db_shows) > 0:
    show = db_shows[0]
